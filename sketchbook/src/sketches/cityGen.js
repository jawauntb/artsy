...
function setup() {
  ...
  var lerpedback = lerpColor(
    color(purNigthSky),
    color(bluesteel),
    random(700) / random(10, 800)
  );
  var backcolor = lerpColor(
    lerpedback,
    color("black"),
    random(3700) / random(16, 4000)
  );
  ...
  for (var k = 0; k < bH - 40; k += 10) {
    for (var j = 0; j < bW - 10; j += (bW - 10) / 6) {
      ...
      createCharAtPositionInColor(
        ...
        height - 100 - bH + 20 + k,
        floor(random(2)),
        color(allSteels[floor(random(allSteels.length))])
      );
...
function addBuildingDecor(bW, rx, ry) {
  ...
  stroke(c);
  strokeWeight(bW * random(5, 9) * 0.03);
...
function addWindowLight(bW, bH, k, j, tmp) {
  ...
  strokeWeight(bW * random(0.04));
  point(pointWidth, pointHeight);
}

function drawSun(val) {
  var color = arbitraryRule(3)
    ? arbitraryRule(5)
      ? arbitraryRule(8)
        ? "yellow"
        : lightYellow
...
function randomFills() {
  for (var i = -0; i < 1400; i++) {
    ...
    fill(
      arbitraryRule(5) ? (arbitraryRule(9) ? lightgrn : techGreen) : "white"
    );
